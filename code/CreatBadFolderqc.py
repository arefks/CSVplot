...
def read_csv_files(files):
    ...
    for ff,file in enumerate(files):
        df = pd.read_csv(file)    
        data_dict[ff] = df[df["Voting outliers (from 5)"]>4] 
...
PP =  r"Z:\2023_Kalantari_AIDAqc\outputs\QC_Final\validation"
...
for tt in new_df["corresponding_img"]: 
    ...
    Path_new_folder = os.path.join(Path_study,"votedImages_Only5")
